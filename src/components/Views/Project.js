...
function Project({ match }) {
  React.useEffect(() => {
    ...
    if(filtered.length === 0){
      setMstate({ ...mstate, projects: [...mstate.projects, { id: match.params.id, name: 'New Value', diagram: [1], length: mstate.projects.length !== 0 ? mstate.projects[mstate.projects.length - 1].diagram.length : 0, date: dateString, collaborator: <Avatar>A</Avatar> }] })
    }
  }, []);
